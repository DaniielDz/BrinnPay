...
/**
 * Dual-mode access boundary for the payment routes (phase 7 §4.3, D11 —
 * mirroring the Phase 6 `CustomersAccessGuard` mechanics).
 *
 ...
 *   check applies (phase 6 D6); the (project, environment) scoping of the data
 *   access itself happens in `PaymentsService`.
 ...
 * evaluated.
 *
 * A shared extraction of this guard is deferred until the third consumer
 * (refunds, Phase 9) exists (D11).
 */
@Injectable()
export class PaymentsAccessGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!requirement) {
      // Programming error: every payment route declares a capability.
      throw new Error('PaymentsAccessGuard requires @RequireCapability on the route handler');
    }
    ...
    if (typeof header === 'string' && header.startsWith('Bearer ')) {
      ...
      if (token.startsWith(API_KEY_PREFIX)) {
        // API-key mode (D8 phase 6): delegate to the API-key authentication
        // boundary, then pin the path project to the key's project.
        await this.apiKeyAuth.canActivate(context);
        return this.authorizeApiKeyMode(request);
      }
    }

    // Session mode (D8 phase 6): delegate to the session authentication
    // boundary, then evaluate the route capability with the phase 5
    // project-RBAC semantics.
  ...
  private async authorizeSessionMode(
    ...
    if (!request.authUser) {
      ...
      throw new Error('PaymentsAccessGuard used without SessionAuthGuard');
    }
